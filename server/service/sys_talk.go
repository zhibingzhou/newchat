...
func FindTalk_List(id int) (err error, messages []response.ResponseMessages_list) {
	...
	err = global.GVA_DB.Debug().Raw("SELECT messages_list.id , messages_list.`updated_at`,messages_list.`type`,messages_list.`online`,messages_list.`msg_text`,contacts.`is_top`,contacts.`not_disturb`,contacts.`friend_remark` AS remark_name ,sys_user.`avatar`,sys_user.`nickname` AS name ,sys_user.`id` AS friend_id FROM messages_list,sys_user,contacts WHERE messages_list.user_id = ? AND messages_list.`type` = 1 AND contacts.`user_id` = messages_list.user_id AND sys_user.`id` = contacts.`friend_id`  AND contacts.`user_id` = ? ORDER BY messages_list.`created_at` DESC ", id, id).Scan(&messages).Error

	err = global.GVA_DB.Debug().Raw("SELECT messages_list.id , messages_list.`updated_at`,messages_list.`type`,messages_list.`online`,messages_list.`msg_text`,group_member.`is_top`,group_member.`not_disturb`,group_list.`avatar`,group_list.`group_name` AS name,messages_list.group_id FROM messages_list,group_list,group_member WHERE messages_list.user_id = ? AND messages_list.`type` = 2 AND group_list.id = messages_list.`group_id` AND group_member.`user_id` = ?  ORDER BY messages_list.`created_at` DESC ", id, id).Scan(&groupmessage).Error
...
func TalkRecords(source, receive_id string, uid int) (err error, records response.ResponseTalkRecords) {
	...
	switch source {
	case "1":
		err = global.GVA_DB.Debug().Raw(" SELECT talk_list.`id`, source,msg_type,user_id,receive_id,is_revoke,content,sys_user.`avatar`,sys_user.`nickname`,talk_list.`created_at` FROM talk_list,sys_user WHERE (receive_id = ? OR receive_id = ?)  AND user_id = sys_user.`id`  AND talk_list.`source` = 1 ORDER BY created_at DESC ", uid, receive_id).Scan(&records.Rows).Error
		break
	case "2":
		err = global.GVA_DB.Debug().Raw(" SELECT talk_list.`id`, source,msg_type,user_id,receive_id,is_revoke,content,sys_user.`avatar`,sys_user.`nickname`,talk_list.`created_at` FROM talk_list,sys_user WHERE receive_id = ? AND user_id = sys_user.`id` AND talk_list.`source` = 2 ORDER BY created_at DESC  ", receive_id).Scan(&records.Rows).Error
		break
	}
	if err != nil {
		return err, records
...
func NotDisturb(id int, rep request.RequestNotDistraub) (err error) {
	...
	switch rep.Type {
	case 1:
		err = global.GVA_DB.Debug().Table("contacts").Where("friend_id =  ? and user_id = ?", rep.Receive_id, id).Updates(map[string]interface{}{
		...
		break
	case 2:
		err = global.GVA_DB.Debug().Table("group_member").Where("group_id =  ? and user_id = ?", rep.Receive_id, id).Updates(map[string]interface{}{
			"not_disturb": rep.Not_disturb,
		}).Error
...
func TalkCreate(id int, rep request.RequestCreate) (err error, mes response.ResponseMessages_list) {
	...
	switch rep.Type {
	case 1:
		err = global.GVA_DB.Debug().Table("contacts").Where("friend_id =  ? and user_id = ?", rep.Receive_id, id).First(&con).Error
		if con.ID < 1 {
			return errors.New("不是好友关系，无法聊天！！"), mes
		}
		result := global.GVA_DB.Debug().Table("messages_list").Where("friend_id =  ?", rep.Receive_id).First(&mes)
		if result.Error != nil {
			if result.Error == gorm.ErrRecordNotFound {
				err = global.GVA_DB.Debug().Raw("SELECT * FROM talk_list WHERE source = ? AND ((user_id = ? AND receive_id = ?) OR (user_id = ? AND receive_id = ?)) ORDER BY created_at DESC LIMIT 1 ", rep.Type, id, rep.Receive_id, rep.Receive_id, id).Scan(&talk).Error
				if err != nil {
					return err, mes
				}
				...
				err = global.GVA_DB.Debug().Table("messages_list").Create(&meg).Error
			}
			return err, mes
		}
		...
		break
	case 2:
		err = global.GVA_DB.Debug().Table("group_member").Where("group_id =  ? and user_id = ?", rep.Receive_id, id).First(&con).Error
		if con.ID < 1 {
			return errors.New("不在该群里面，无法聊天！！"), mes
		}
		result := global.GVA_DB.Debug().Table("messages_list").Where("group_id =  ?", rep.Receive_id).First(&mes)
		if result.Error != nil {
			if result.Error == gorm.ErrRecordNotFound {
				err = global.GVA_DB.Debug().Raw("SELECT * FROM talk_list WHERE source = ? AND receive_id = ? ORDER BY created_at DESC LIMIT 1 ", rep.Type, rep.Receive_id).Scan(&talk).Error
				if err != nil {
					return err, mes
				}
