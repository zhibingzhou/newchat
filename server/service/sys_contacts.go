...
func Apply_records(id, page, pageSize int) (err error, rep []model.Apply_records, total, page_total int) {
	...
	err = global.GVA_DB.Debug().Raw("SELECT apply_records.id,status,user_id,remarks,friend_id,apply_records.`created_at`,sys_user.`nickname`,sys_user.`mobile`,sys_user.`avatar` FROM apply_records , sys_user WHERE apply_records.`friend_id` = 12 AND apply_records.`user_id` = sys_user.`id` ").Limit(pageSize).Offset(page - 1).Scan(&rep).Error
	page_total = len(rep)
	total = page_total
	return err, rep, total, page_total
}
...
func Contacts_List(id int) (err error, rep []response.Response_Contacts_list) {
	...
	err = global.GVA_DB.Debug().Raw("SELECT friend_id from contacts where user_id = ? ", id).Scan(&f_id).Error
	for _, value := range f_id {
		f_ids = append(f_ids, value.Friend_id)
	}

	err = global.GVA_DB.Debug().Raw("SELECT sys_user.id AS id , sys_user.nickname AS nickname ,sys_user.gender as gender, sys_user.motto as motto, sys_user.avatar as avatar , contacts.`friend_remark` as friend_remark FROM  sys_user,contacts WHERE sys_user.`id` = contacts.`friend_id` AND contacts.`friend_id` in (?)", f_ids).Scan(&rep).Error
